    ...
    for(int x=min_x;x<max_x;x++){
        for(int y=min_y;y<max_y;y++){
            ...
            if(insideTriangle(x, y, _v)){
                ...
                float z = 1.0/(alpha/A.w()+beta/B.w()+gamma/C.w());
                float zp = alpha*A.z()/A.w()+beta*B.z()/B.w()+gamma*C.z()/C.w();
                zp*=z;
                if(zp<depth_buf[index]){
                    depth_buf[index]=zp;

                    Vector3f interpolated_normal = alpha*t.normal[0]/A.w()+beta*t.normal[1]/B.w()+gamma*t.normal[2]/C.w();
                    ...
                    Vector2f interpolated_texcoords = 
